...
**MT2.** Triển khai backend NestJS kết hợp MongoDB với đầy đủ 13 module nghiệp vụ, tích hợp cổng thanh toán Stripe và dịch vụ giao hàng GHN [ref: f2t-backend/src/modules/ — 13 thư mục: admin, auth, delivery, demand-forecasting, dynamic-pricing, farms, notifications, orders, payments, posts, products, uploads, users]. Hệ thống phải đáp ứng khoảng 79 REST endpoint phục vụ ba vai trò người dùng [ref: ledger t1.15-numbers].
...
Hệ thống F2T (Farm-to-Table) trong phạm vi khóa luận này bao gồm ba thành phần chính: (1) ứng dụng di động đa nền tảng iOS/Android xây dựng bằng React Native + Expo; (2) backend REST API NestJS triển khai trên môi trường Node.js với MongoDB làm hệ quản trị cơ sở dữ liệu; và (3) một sidecar FastAPI duy nhất — pricing-sidecar — chạy tại port 8000 và phục vụ ba endpoint AI/ML: /predict (định giá DDQN), /forecast (dự báo ForecasterLSTM), và /freshness/classify (phân loại CoreML) [ref: f2t-backend/src/app.module.ts:57; ledger t1.4-one-sidecar].

Kiến trúc Monolith + 1 Sidecar này được lựa chọn có chủ đích: toàn bộ logic Python AI/ML được cô lập trong một tiến trình FastAPI duy nhất, tách biệt hoàn toàn khỏi runtime Node.js, đồng thời backend NestJS được cấu hình để graceful degradation khi sidecar không phản hồi.
...
**ĐG1 — Kiến trúc Monolith + 1 Sidecar tích hợp AI/ML.** Thay vì triển khai ba dịch vụ Python riêng biệt cho từng nhiệm vụ AI, toàn bộ logic học máy được đóng gói vào một FastAPI sidecar duy nhất (pricing-sidecar, port 8000) với ba endpoint: /predict, /forecast, và /freshness/classify. Backend NestJS giao tiếp với sidecar qua HTTP nội bộ và thực hiện graceful degradation tự động khi sidecar không khả dụng [ref: f2t-backend/src/common/interceptors/dynamic-pricing.interceptor.ts:9; ledger t1.4-one-sidecar]. Thiết kế này giảm độ phức tạp triển khai, tập trung hóa quản lý phụ thuộc Python, và cho phép mở rộng hoặc thay thế từng model AI mà không ảnh hưởng đến luồng nghiệp vụ chính.
...
**Chương 4 — Cài đặt và kiểm thử** mô tả môi trường triển khai thực tế, quy trình cài đặt và khởi tạo dữ liệu mẫu, kết quả kiểm thử unit (54/54 test case trong 21 file spec) [ref: ledger t1.15-numbers], đánh giá chức năng (≈79 endpoint REST), đánh giá định lượng ba thành phần AI/ML (ForecasterLSTM offline eval, DDQN reward curve, CoreML accuracy), và demo luồng end-to-end.

**Chương 5 — Kết luận và hướng phát triển** tổng kết các kết quả đạt được so với sáu mục tiêu đề ra, trình bày ba giới hạn hiện tại của hệ thống một cách trung thực (serve tile-21× của forecaster, lệch pha day-of-week nhỏ <6.2%, và phạm vi 2/4 danh mục có model CoreML riêng), và đề xuất hướng phát triển tiếp theo cho từng thành phần kỹ thuật.
